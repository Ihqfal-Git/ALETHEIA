...
const deviceMapping = {
  laptop: { nama: 'Laptop', emoji: '💻', data: gejalaLaptop },
  hp: { nama: 'Handphone', emoji: '📱', data: gejalaHP },
  pc: { nama: 'PC / Desktop', emoji: '🖥️', data: gejalaPC },
};
...
export default function DiagnosaPage() {
  ...
  const { nama, emoji, data } = deviceInfo;
  ...
  return (
    <div className="space-y-8 pb-24">
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="fixed bottom-0 left-0 right-0 border-t border-neutral-200 bg-white/95 backdrop-blur py-4 px-6 md:px-12 z-30 shadow-lg flex items-center justify-between md:justify-end gap-6">
          <div className="text-left md:text-right">
            <p className="text-[10px] text-neutral-400 font-bold uppercase tracking-wider">Hasil Pemilihan</p>
            <p className="text-sm font-black text-neutral-950">{totalSelected} Gejala dipilih</p>
          </div>
          <button
            type="submit"
            disabled={totalSelected === 0 || submitting}
            className={`
              px-6 py-2.5 rounded-lg text-xs font-bold tracking-wide transition-all cursor-pointer flex items-center gap-2
              ${totalSelected === 0
